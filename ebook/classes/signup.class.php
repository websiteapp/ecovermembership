...
class SignUp extends Generic {
	...
	public function getPost($var) {
		...
		if ( !empty($_SESSION['eb']['openIDMisc']) ) :

			$misc = $_SESSION['eb']['openIDMisc'];
		...
		if ( !empty($_SESSION['eb']['facebookMisc']) ) :

			$misc = $_SESSION['eb']['facebookMisc'];
		...
		if ( !empty($_SESSION['eb']['twitterMisc']) ) :

			$misc = $_SESSION['eb']['twitterMisc'];
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$_SESSION['eb']['username'] = $this->settings[$this->username_type];

			/* Apply default user_level */
			$_SESSION['eb']['user_level'] = unserialize(parent::getOption('default-level'));

			if ( $requireActivate )
				$_SESSION['eb']['activate'] = 1;

			$_SESSION['eb']['gravatar'] = parent::get_gravatar($this->settings['email'], true, 26);
			...
			$_SESSION['eb']['user_id'] = $user_id;

			/* Social integration. */
			if ( !empty($_SESSION['eb']['facebookMisc']) ) {
				$link = 'facebook';
				$id = $_SESSION['eb']['facebookMisc']['id'];
			}

			if ( !empty($_SESSION['eb']['openIDMisc']) ) {
				$link = $_SESSION['eb']['openIDMisc']['type'];
				$id = $_SESSION['eb']['openIDMisc'][$link];
			}

			if ( !empty($_SESSION['eb']['twitterMisc']) ) {
				$link = 'twitter';
				$id = $_SESSION['eb']['twitterMisc']['id'];
			}
