...
class Check extends Generic {
	...
	private function isGuest($forceLogin) {
		...
		if ( !$forceLogin )
			return empty( $_SESSION['eb']['user_id'] );
		...
		if ( empty($_SESSION['eb']['user_id']) ) :
			...
			$_SESSION['eb']['referer'] = $_SERVER['REQUEST_URI'];
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['eb']['restricted']) || !empty($_SESSION['eb']['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['eb']['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'profile.php') :
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['eb']['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	public function protectThis($level) {
		...
		*/
		...
		if ( empty( $_SESSION['eb']['user_id'] ) && $level == array('*') )
			return false;
