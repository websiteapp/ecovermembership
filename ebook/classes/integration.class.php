...
class Jigowatt_integration extends Generic {
	...
	private function retrieveInfo() {
		...
		$params = array( ':user_id' => $_SESSION['eb']['user_id'] );
	...
	private function connect($link) {
		...
		$params = array(
			':user_id'      => $_SESSION['eb']['user_id'],
			':session_link' => $_SESSION['eb'][$link]
		);
			...
			if(!$openid->mode) {
				...
				if ($openid->validate()) :
					...
					if( !empty($providers[$link]) ) :
						...
						$_SESSION['eb'][$link] = $openid->identity;
						$this->connect($link);
						$_SESSION['eb']['openIDMisc'] = $openid->getAttributes();
						$_SESSION['eb']['openIDMisc']['type'] = $link;
						$_SESSION['eb']['openIDMisc'][$link] = $openid->identity;
	...
	public function facebook_url() {
		...
		if ($this->fbUser) :
		  ...
		  try {
			$_SESSION['eb']['facebookMisc'] = $user_profile;
			$_SESSION['eb']['facebook'] = $this->fbUser;
	...
	public function twitter_url() {
		...
		if( !empty($_GET['oauth_token']) ) {
			...
			$_SESSION['eb']['twitter'] = $twitterInfo['id'];
			$_SESSION['eb']['twitterMisc']['username'] = $twitterInfo['screen_name'];
			$_SESSION['eb']['twitterMisc']['id'] = $twitterInfo['id'];
			$_SESSION['eb']['twitterMisc']['name'] = $twitterInfo['name'];

		}

		return !empty($_SESSION['eb']['twitter']) ? '#' : $this->twitterObj->getAuthenticateUrl( null,array('oauth_callback' => str_replace("www.","", SITE_PATH) . basename($_SERVER['PHP_SELF']) . '?link=twitter' ));
