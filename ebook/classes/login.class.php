...
class Login extends jigowatt_integration {
	...
	function __construct() {
		...
		if ( parent::getOption('signin-redirect-referrer-enable') )
			$_SESSION['eb']['referer'] = (!empty($_SESSION['eb']['referer'])) ? $_SESSION['eb']['referer'] : 'home.php';
		else
			$_SESSION['eb']['referer'] = parent::getOption('signin-redirect-url');
	...
	private function social_login($provider) {
		...
		$params = array( ':session' => $_SESSION['eb'][$provider] );
	...
	public function login() {
		...
		if ( parent::getOption('user-activation-enable') ) :
			...
			if ($count > 0) $_SESSION['eb']['activate'] = 1;

		endif;

		// Save if user is restricted
		if ( !empty($this->result['restricted']) ) $_SESSION['eb']['restricted'] = 1;
		...
		$_SESSION['eb']['user_level'] = $user_level;

		$_SESSION['eb']['email'] = $this->result['email'];

		$_SESSION['eb']['gravatar'] = parent::get_gravatar($this->result['email'], true, 26);
		...
		if ( !empty($disRow['level_disabled']) ) $_SESSION['eb']['level_disabled'] = 1;
		...
		$username = $this->username_type;
		$_SESSION['eb']['username'] = $this->result[$username];

		// User ID of the logging in user
		$_SESSION['eb']['user_id'] = $this->result['user_id'];

		if ( empty($redirect) ) $redirect = $_SESSION['eb']['referer'];

		unset(
			$_SESSION['eb']['referer'],
			$_SESSION['eb']['token'],
			$_SESSION['eb']['facebookMisc'],
			$_SESSION['eb']['twitterMisc'],
			$_SESSION['eb']['openIDMisc']
		);
